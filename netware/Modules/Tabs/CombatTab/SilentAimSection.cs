...
public static class SilentAimSection
{
    ...
    private static void DrawTargeting()
    {
        ...
        // aim bone
        var aimBone = Menu.NewDropdown("Aim Bone", "AimBotAimBone", Config.Active.SilentAim.Bone.ConvertToString(), Combat.AimBones.Keys);
        Config.Active.SilentAim.Bone = Combat.AimBones[aimBone];

        // aim filter
        var aimFilter = Menu.NewDropdown("Filter By", "AimBotAimFilter", Config.Active.SilentAim.Filter.ConvertToString(), Combat.AimFilters.Keys);

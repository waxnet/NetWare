...
public class Main : MonoBehaviour
{
    public void Start()
    {
        ...
        StartCoroutine(Config.UpdateConfigs());

        // setup window data
        Menu.Version = AssemblyConstants.CheatVersion;
        Menu.Color = Color.red;
        Menu.WindowRect = new Rect(WindowConstants.PositionX, WindowConstants.PositionY, WindowConstants.Width, WindowConstants.Height);

        var menuTabs = SourceUtils.GetTypesWithAttribute<MenuTabAttribute>()
            .Where(x => !x.Type.IsAbstract)
            .OrderBy(x => x.Attribute.Order)
            .Select(x => x.Type);

        Menu.Tabs = SourceUtils.CreateInstancesAs<MenuTab>(menuTabs).ToArray();

...
public sealed class Aimbot : MonoBehaviour
{
    public void Update()
    {
        ...
        var fovSize = Config.Active.Aimbot.DynamicFov ?
            (Camera.main.fieldOfView + 60) :
            (
                Config.Active.Aimbot.CheckFov ? Config.Active.Aimbot.FovSize : Screen.width
            );
        ...
        foreach (var player in NetWare.Data.Storage.Players)
        {
            ...
            if (Config.Active.Aimbot.Filter == EAimFilter.FovAndClosest && worldDistance < lastDistanceWorld)
            {
                lastDistanceWorld = worldDistance;
                target = player;
            } else if (Config.Active.Aimbot.Filter == EAimFilter.Fov && screenDistance < lastDistanceScreen) {
        ...
        // aim at player
        if (Config.Active.Aimbot.Mode == EAimMode.Legit && Position.IsOnScreen(playerScreenPosition))
        {
            Mouse.MoveTo(
        ...
        } else if (Config.Active.Aimbot.Mode == EAimMode.Lock) {
